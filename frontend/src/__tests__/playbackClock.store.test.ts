...
describe('playbackClock store', () => {
  it('subscribes and updates currentSimTime on frame', () => {
    let notified = 0
    const unsub = playbackStore.subscribe(() => { notified += 1 })
  ...
  it('setControls wires through play/pause/seek', () => {
    const play = vi.fn(); const pause = vi.fn(); const seek = vi.fn()
    playbackStore.setControls({ play, pause, seek })
    playbackStore.play(); playbackStore.pause(); playbackStore.seek('2024-01-01T00:01:00Z')
