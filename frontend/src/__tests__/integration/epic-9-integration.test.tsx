...
describe('Epic 9: Complete BFF Integration', () => {
  ...
  beforeEach(async () => {
    ...
    vi.mocked(featureFlagService.getDebugInfo).mockReturnValue({
      ...
      lastEvaluations: {
        ...
        websocket: {
          enabled: true,
          endpointUrl: 'ws://127.0.0.1:8001/api/v1/backtests/{id}/stream',
          source: 'bff',
        },
      },
  ...
  describe('Complete BFF Mode Integration', () => {
    ...
    it('should coordinate API and WebSocket through BFF', async () => {
      ...
      // Test run data hook
      const { result: runResult } = renderHook(() => useBacktestDetail('test-run-123'), { wrapper })

      // Test WebSocket health
      const { result: wsResult } = renderHook(() => useWebSocketHealth('test-run-123'), { wrapper })
    ...
    it('should demonstrate API call reduction across all services', async () => {
      ...
      const { result: chartResult } = renderHook(() => useDailyChartData('AAPL'), { wrapper })

      const { result: runListResult } = renderHook(() => useBacktestList({ symbol: 'AAPL' }), {
        wrapper,
      })

      const { result: runDetailResult } = renderHook(() => useBacktestDetail('test-run-123'), {
        wrapper,
      })
  ...
  describe('Fallback and Error Handling', () => {
    ...
    it('should handle mixed BFF/backend mode gracefully', async () => {
      ...
      const { featureFlagService } = await import('../../services/featureFlags')
      vi.mocked(featureFlagService.isFeatureFlagEnabled).mockImplementation(
        (flag: string) => flag === 'chartData',
      )
      ...
      vi.mocked(featureFlagService.evaluateFeatureFlag).mockImplementation((flag: any) => {
        ...
        if (flag === 'runData')
          return {
            enabled: false,
            endpointUrl: 'http://127.0.0.1:8000/backtests',
            source: 'backend',
          }
        if (flag === 'websocket')
          return {
      ...
      const { result: runResult } = renderHook(() => useBacktestList(), { wrapper })
      ...
      await waitFor(() => {
        const calledWithBackend = mockFetch.mock.calls.some((c) =>
          String(c[0]).includes('127.0.0.1:8000/backtests'),
        )
    ...
    it('should achieve overall performance improvements with BFF', async () => {
      ...
      mockFetch.mockImplementation((url: string) => {
          ...
          return Promise.resolve({
            ...
            json: () =>
              Promise.resolve({
                ...
                timeframe: 'daily',
                bars: [],
                meta: { source: 'bff', optimized: true, responseTime: 15 },
              }),
          } as Response)
        }
        if (url.includes('/api/v1/backtests/') && url.includes('/complete')) {
        ...
        return Promise.resolve({
          ok: true,
          json: () =>
            Promise.resolve({ meta: { source: 'bff', optimized: true, responseTime: 15 } }),
        } as Response)
      })
      ...
      const { result: runDetailResult } = renderHook(() => useBacktestDetail('test-run'), {
        wrapper,
      })

      const { result: wsHealthResult } = renderHook(() => useWebSocketHealth('test-run'), {
        wrapper,
      })
