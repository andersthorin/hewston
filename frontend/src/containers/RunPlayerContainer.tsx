...
export type RunPlayerContainerProps = {
  backtest_id: string
  dataset_id?: string
  run_from?: string
  run_to?: string
...
function RunPlayerContainer({
  backtest_id,
  dataset_id,
  run_from,
  run_to,
}: RunPlayerContainerProps) {
  ...
  useEffect(() => {
    playbackStore._setPlaying(state.playing)
  }, [state.playing])
  ...
  const toBusinessDay = useCallback(
    (d: Date) => ({ year: d.getUTCFullYear(), month: d.getUTCMonth() + 1, day: d.getUTCDate() }),
    [],
  )
  ...
  useEffect(() => {
    setRunFrom(normalizeBound(run_from, false))
  }, [run_from])
  useEffect(() => {
    setRunTo(normalizeBound(run_to, true))
  }, [run_to])
  ...
  useEffect(() => {
    const unsub = subscribe((frame: StreamFrameT) => {
      try {
        ...
        const hasSnapshots = !!(
          dailySnapshotsRef.current &&
          dayKeysRef.current &&
          dayKeysRef.current.length > 0
        )
        ...
        if (tsStr) {
          const y = parseInt(tsStr.slice(0, 4)),
            m = parseInt(tsStr.slice(5, 7)),
            d = parseInt(tsStr.slice(8, 10))
          if (hasOhlc) {
            const timeVal: Time =
              viewMode === 'daily'
            ...
            console.debug('[RunPlayer] frame branch=streaming-ohlc', {
              ts: tsStr,
              dp,
            ...
            console.debug('[RunPlayer] frame branch=snapshots', {
              ts: tsStr,
              day,
              snaps: snaps.length,
              seeded: seededRef.current,
            ...
            console.debug('[RunPlayer] frame branch=none (snapshots not ready - unexpected)', {
              ts: tsStr,
            })
          }
        }
  ...
  const dailySnapshotsRef = useRef<Map<
    string,
    Array<{ o: number; h: number; l: number; c: number; t: string }>
  > | null>(null)
  ...
  useEffect(() => {
    ...
    if (!hourResp?.bars) {
      dailySnapshotsRef.current = null
      dayKeysRef.current = null
      return
    }
    if (!runFrom || !runTo) {
    ...
    console.debug('[RunPlayer] snapshots built', {
      days: keys.length,
      first: keys[0],
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="inline-flex rounded border border-slate-300 overflow-hidden text-xs">
            <button
              className={`px-2 py-1 ${viewMode === 'daily' ? 'bg-slate-200' : ''}`}
              onClick={() => setViewMode('daily')}
            >
              Daily
            </button>
            <button
              className={`px-2 py-1 ${viewMode === 'hourly' ? 'bg-slate-200' : ''}`}
      ...
      <div className="grid grid-cols-1 gap-4">
        ...
        <TimelineScrubber />
        {!runFrom || !runTo ? (
          <div className="text-sm text-slate-500">Waiting for frames…</div>
        ) : null}
        {isHourLoading ? <div className="text-sm text-slate-500">Loading hourly data…</div> : null}
        {isHourErr ? (
          <div className="text-sm text-amber-600">No hourly data for {symbol} in this range.</div>
        ) : null}
      </div>
    </div>
  )
