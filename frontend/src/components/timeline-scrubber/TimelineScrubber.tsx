...
export default function TimelineScrubber({ stepSeconds = 60, timeZone = 'America/New_York' }: TimelineScrubberProps) {
  ...
  return (
    <div className="w-full">
      ...
      >
        <div className="absolute top-0 left-0 h-3 rounded bg-indigo-400" style={{ width: `${pct}%` }} />
...
function Marker({ ts, range }: { ts: string; range: { start: string | null; end: string | null } }) {
  const left = useMemo(() => {
