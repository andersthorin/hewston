# Story 6.1 — Backtests List view (presentational UI)
Story ID: S6.1
Epic ID: E6
...
- I want a Backtests List view that fetches and displays backtests with pagination and filters,
- So that I can browse existing backtests and open one to review/play back.
...
Dependencies
- Depends on: S2.2 (List backtests API), S1.1 (FE skeleton if applicable)
- Blocks: S6.2 (Backtest Detail playback UI)
...
Acceptance Criteria
1) GET /backtests is called via services/api.ts using TanStack Query; success renders a table with columns: backtest_id, created_at, strategy_id, status, symbol, run_from, run_to, duration_ms
...
- Table behavior:
  ...
  - "View" button navigates to Backtest Detail route `/backtests/{backtest_id}` (router setup in FE skeleton)
- Validation:
  - Zod schema for BacktestSummary and list response; map server fields (run_from/run_to) to TS types

Tasks / Subtasks
- Define Zod schemas for BacktestSummary and ListResponse
- Implement listBacktests(query) in services/api.ts using fetch
- Build BacktestsTable presentational component (no hooks)
- Build BacktestsListContainer to fetch and pass props to BacktestsTable; wire FiltersBar
- Add route and link from table rows to Run Detail
- Run ESLint/Prettier; verify no warnings

Testing & Validation
- With empty DB: see empty state
- After sample rows exist: table shows backtests; pagination/filters update results

Definition of Done
- Backtests List displays backtests with basic filters/pagination; adheres to presentational component rule; type-safe with Zod.


QA Results
- Gate: PASS
- Summary: Backtests List presentational UI implemented; data via TanStack Query; manual build OK.
