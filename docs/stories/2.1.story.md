...
Acceptance Criteria
1) `make db-apply` initializes data/catalog.db; adapter connects with foreign_keys=ON
...
- Implement adapters/sqlite_catalog.py:
  - Connect to data/catalog.db (path configurable later)
...
- rm data/catalog.db (tool) → recreated cleanly
- make db-apply → 0
- make test → 7 passed
