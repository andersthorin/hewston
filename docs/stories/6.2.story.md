...
- I want a Run Detail view that plays back a completed run with chart and orders/fills overlays,
...
4) Charts: OHLC (Lightweight Charts) + orders/fills overlays
5) SSE fallback toggle (if WS fails) switches to GET /backtests/{id}/stream?speed=60 and continues playback (read-only)
6) Components are presentational (Chart and Controls); containers/services handle WS/SSE and state
...
- Build presentational components: PlaybackControls, ChartOHLC
...
- Evidence: Frontend build success; ChartOHLC rendering from stream frames.
