...
def test_post_backtests_idempotent(tmp_path, monkeypatch):
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        row = conn.execute("SELECT status FROM backtests WHERE backtest_id = ?", (run_id,)).fetchone()
