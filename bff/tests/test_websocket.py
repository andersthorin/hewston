...
class TestWebSocketConnectionManager:
    ...
    async def test_subscription_management(self):
        ...
        with patch.object(manager, "_ensure_backend_connection", new_callable=AsyncMock):
            ...
            # Act - Unsubscribe
            unsubscribe_message = {"type": "unsubscribe", "run_id": run_id}
            await manager.handle_client_message(connection_id, json.dumps(unsubscribe_message))

            # Assert - Subscription removed
            assert (
                run_id not in manager.run_subscriptions
                or connection_id not in manager.run_subscriptions[run_id]
            )
...
class TestWebSocketModels:
    ...
    def test_subscribe_message_validation(self):
        ...
        from bff.models.websocket import MessageType, SubscribeMessage

        # Valid message
        message = SubscribeMessage(run_id="test-run-123", updates=["run_status", "metrics"])
    ...
    def test_error_message_creation(self):
        ...
        error = ErrorMessage(
            error_code="TEST_ERROR", error_message="Test error message", run_id="test-run-123"
        )
    ...
    def test_connection_status_message(self):
        ...
        from bff.models.websocket import ConnectionStatus, ConnectionStatusMessage, MessageType
    ...
    def test_run_update_message(self):
        ...
        from bff.models.websocket import MessageType, RunUpdateMessage
...
class TestWebSocketIntegration:
    ...
    async def test_client_disconnect_cleanup(self):
        ...
        with patch.object(manager, "_ensure_backend_connection", new_callable=AsyncMock):
            ...
            assert (
                run_id not in manager.run_subscriptions
                or len(manager.run_subscriptions[run_id]) == 0
            )
