...
class TestBacktestsProxy:
    ...
    def test_proxy_create_backtest_success(self, test_client: TestClient, mock_backend_client):
        ...
        request_body = {"strategy_id": "sma_crossover", "params": {"fast": 10, "slow": 20}}

        # Act
        response = test_client.post(
            "/api/v1/backtests", json=request_body, headers={"Idempotency-Key": "test-key-123"}
        )
...
class TestBarsProxy:
    ...
    def test_proxy_get_hour_bars_success(self, test_client: TestClient, mock_backend_client):
        ...
        # Act
        response = test_client.get(
            "/api/v1/bars/hour", params={"symbol": "AAPL", "from": "2024-01-01", "to": "2024-01-01"}
        )
