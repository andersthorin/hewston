...
class ChartDataRequest(BaseModel):
    ...
    target_points: int | None = Field(
        default=10000, ge=100, le=50000, description="Target number of data points for decimation"
    )
    rth_only: bool = Field(
        default=True, description="Regular trading hours only (for intraday data)"
    )
...
class BarData(BaseModel):
    ...
    volume: int | None = Field(default=0, description="Volume")
...
class ResponseMetadata(BaseModel):
    ...
    decimation_stride: int | None = Field(
        default=None, description="Decimation stride used (if decimated)"
    )
...
class ChartDataError(BaseModel):
    ...
    symbol: str | None = Field(default=None, description="Symbol that caused error")
    timeframe: str | None = Field(default=None, description="Timeframe that caused error")
