"""DonchianBreakoutStrategy (Epic 17)

Simple Donchian channel breakout using close price as proxy for H/L (MVP):
- BUY when close > max(close[-n:]); SELL when in position and close < min(close[-n:])
"""
...
try:  # pragma: no cover
    ...
    class DonchianBreakoutStrategy(Strategy):  # type: ignore[misc]
        ...
        def on_bar(self, bar) -> None:
            ...
            if len(self._closes) > self.window:
                self._closes.pop(0)
            if not in_rth or len(self._closes) < self.window:
                return
            hi = max(self._closes)
            lo = min(self._closes)
            if (not self._in_position) and px > hi:
                self._place(OrderSide.BUY, px, ts)
            elif self._in_position and px < lo:
    ...
    class DonchianBreakoutStrategy:
        ...
        def __repr__(self) -> str:
            return f"DonchianBreakoutStrategy(window={self.window}, instrument_id={self.instrument_id})"
