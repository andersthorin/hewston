...
def _annualization_factor(bar_minutes: int, minutes_per_session: int = 390, sessions_per_year: int = 252) -> float:
    if bar_minutes <= 0:
        return float('nan')
...
def compute_cumulative_metrics(
    ...
    *,
    ...
    realized_idx = -1
    last_realized_val: Optional[float] = None
    wins = 0
    losses = 0
