...
class TestGitUtilities:
    ...
    def test_get_git_commit_hash_success(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(returncode=0, stdout=mock_hash, stderr="")
    ...
    def test_get_git_branch_success(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(returncode=0, stdout=mock_branch, stderr="")
    ...
    def test_is_git_repo_true(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(returncode=0, stdout="/path/to/repo/.git", stderr="")
    ...
    def test_git_functions_strip_whitespace(self):
        ...
        with patch("subprocess.run") as mock_run:
            # Test commit hash
            mock_run.return_value = MagicMock(
                returncode=0, stdout=mock_hash_with_whitespace, stderr=""
            )
            ...
            # Test branch name
            mock_run.return_value = MagicMock(
                returncode=0, stdout=mock_branch_with_whitespace, stderr=""
            )
